...
import Animated, { Easing, Extrapolate, interpolate, runOnJS, useAnimatedStyle, useDerivedValue, useSharedValue, withTiming } from 'react-native-reanimated';
...
const BookPagePortrait = /*#__PURE__*/React.forwardRef(({
  ...
  const panGesture = Gesture.Pan().enabled(enabled && !isAnimating).onStart(() => {
  ...
  }), current ? /*#__PURE__*/React.createElement(IPage, _extends({
    key: 'current-' + containerWidth,
    page: current,
    backPage: next,
    right: true
  ...
  }), prev && /*#__PURE__*/React.createElement(IPage, _extends({
    key: 'prev-' + containerWidth,
    page: prev,
    backPage: current,
    right: false
  }, iPageProps)))));
...
const IPage = ({
  ...
  const portraitBackStyle = useAnimatedStyle(() => {
    ...
    const x = interpolate(rotationVal.value, [0, 180], [cw, -cw / 2]);
    const w = interpolate(rotationVal.value, [0, 180], [0, cw / 2]);
    return {
      width: Math.ceil(w),
      zIndex: 2,
      opacity: 1,
  ...
  const portraitFrontStyle = useAnimatedStyle(() => {
    const cw = containerWidthSV.value;
    const w = interpolate(rotationVal.value, [0, 160], [cw, -20], Extrapolate.CLAMP);
    const style = {
      zIndex: 1,
  ...
  }, renderPage(backPage ? backPage.left : page.left))), /*#__PURE__*/React.createElement(BackShadow, {
    degrees: rotationVal,
    right: true
