...
const PageFlipper = /*#__PURE__*/React.forwardRef(({
  ...
  const deferredPageIndex = useRef(null);
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const deferred = deferredPageIndex.current;
        deferredPageIndex.current = null;
        setState({
          initialized: true,
          pages: allPages,
          ...
          isPortrait: portrait,
          nextPageIndex: deferred !== null && deferred !== adjustedIndex ? deferred : undefined
        });
        if (onInitialized) {
          onInitialized({
            pages: allPages,
  ...
  const goToPageDeferred = useCallback(index => {
    deferredPageIndex.current = index;
  }, []);
  React.useImperativeHandle(ref, () => ({
    ...
    previousPage,
    goToPageDeferred
  }), [goToPage, nextPage, previousPage, goToPageDeferred]);
  ...
  }, renderPage((next || current).right))), /*#__PURE__*/React.createElement(View, {
    style: {
      ...StyleSheet.absoluteFillObject
    }
