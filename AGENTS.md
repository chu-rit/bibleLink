...
## React Native 규칙
- React Native Web 호환성을 항상 우선 고려한다.
...
## 퍼즐 데이터 규칙
- 퍼즐판과 단어를 수정하기 전에 `data/crosswordMap1.json`의 `grid`, `cells`, `row`, `col`, `length` 관계를 확인한다.
- 정답과 퍼즐판의 글자 수가 일치하는지 확인한다.
...
## 단어 데이터 규칙
- `data/bibleWordsDifficulty1.json`을 단어 정보의 기준 데이터로 사용한다.
