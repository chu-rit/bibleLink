...
- 단어 정의 수정 규칙:
  ...
  - 가능하면 디테일한 정의를 덧붙인다.
  - 일상용어에 가까운 단어는 해당 단어가 들어간 성구 본문을 사용하되, 정답 단어는 글자 수에 맞춰 `O`로 전체 마스킹한다 (예: 2글자→`OO`, 3글자→`OOO`).
  - 생소한 단어 중 어려워 보이는 단어에만 괄호 안에 부분 힌트를 추가한다. 익숙한 단어는 힌트 없이 디테일한 정의만으로 충분하다. `O` 개수는 정답 글자 수와 정확히 일치하되 3글자는 1글자만, 4~5글자는 2글자만 공개. 공개 위치는 첫 글자에 고정하지 않고 랜덤으로. 힌트 예: 보세스(3글자)→`O세O`, 스보이임(4글자)→`스O이O`.
...
- 모든 난이도 공통: 8×8, 10단어. 난이도는 단어 difficulty 평균으로 구분 (EASY 1.0~1.8, NORMAL 1.9~2.5, HARD 2.6 이상).
- `clue`가 비어 있으면 `definition` 사용. `clue`==`definition`이면 중복 저장 안 함. 기존 배치 유지 시 `clue`만 변경.
